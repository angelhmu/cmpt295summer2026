...
void decode_utf8(const char* s) {
    ...
    while (s[i] != '\0') {
        ...
        if ((s[i] & 0x80) == 0x00) {            // 0xxxxxxx
            ...
            i += 2;
        } else if ((s[i] & 0xF0)) {             // 1110xxxx 10xxxxxx 10xxxxxx
            // the three-byte case
            ...
            i += 3;
        } else if (s[i] & 0xF8) {               // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
            // the four-byte case
            val = ((uint8_t)s[i] & 0x0F) << 18; // remove first 4 bits (prefix), shift left by 12
            val |= ((uint8_t)s[i+1] & 0x3F) << 12;// remove first 2 bits (prefix), shift left by 6
            val |= ((uint8_t)s[i+2] & 0x3F) << 6;    // remove first 2 bits (prefix)
            val |= ((uint8_t)s[i+3] & 0x3F);
